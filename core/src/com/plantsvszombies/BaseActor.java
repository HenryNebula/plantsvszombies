...
public class BaseActor extends Actor
{
    ...
    private static String getFullClassName (String className) {
        ...
        if (! className.contains("com")){
            fullClassName = "com.plantsvszombies." + fullClassName;
        }
    ...
    protected String[] generateFilesFromDirectory(String directory, String prefix) {
        directory = String.format(prefix,  directory);
        int frameNumber = new File(directory).list().length;
        StringBuilder stringBuffer = new StringBuilder();
        for (int i=0; i<frameNumber; i++){
            stringBuffer.append(String.format(directory + "frame_%02d.gif\t", i));
        }
        return stringBuffer.toString().split("\t");
    }

    public static Sound getSoundFromFile(String name) {
        return Gdx.audio.newSound(Gdx.files.internal(String.format("core/assets/audio/%s.mp3", name)));
    }

    public static Music getMusicFromFile(String name) {
        return Gdx.audio.newMusic(Gdx.files.internal(String.format("core/assets/audio/%s.mp3", name)));
    }
